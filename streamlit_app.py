...
class Plotter:
    ...
    def plot_fig(self):
        ...
        num_polygons = (
            max(1, self.data.n - 2)
            if hasattr(self.data, "n") and self.data.n >= 3
            else 0
        )
        num_bezier_pts = (
            max(1, len(self.data.bezier_pts)) if hasattr(self.data, "bezier_pts") else 0
        )
        ...
        try:  # Generate the colormaps
            ...
            colors2 = (
                cmap(np.linspace(0.2, 1, num_bezier_pts)) if num_bezier_pts > 0 else []
            )
            ...
            colors = (
                default_cmap(np.linspace(0.2, 1, num_polygons))
                if num_polygons > 0
                else []
            )
            colors2 = (
                default_cmap(np.linspace(0.2, 1, num_bezier_pts))
                if num_bezier_pts > 0
                else []
            )
    ...
    def plot_polygon(self, colors):
        if (
            not self.show_polygone
            or not hasattr(self.data, "df_2")
            or self.data.df_2 is None
        ):
            return
        ...
        if self.rempli_polygone:
            ...
            for z in sorted(patches_by_zorder.keys()):
                ...
                pc = PatchCollection(
                    [plt.Polygon(p) for p in data["polys"]],
                    facecolors=data["colors"],
                    alpha=1,
                    zorder=z,
                )
        ...
        if self.contour:
            for i in self.data.ns:
                if f"{i}-gon X" in self.data.df_2 and f"{i}-gon Y" in self.data.df_2:
                    self.ax.plot(
                        self.data.df_2[f"{i}-gon X"].dropna(),
                        self.data.df_2[f"{i}-gon Y"].dropna(),
                        color=self.couleur,
                        lw=self.line_thickness,
                        alpha=1,
                        zorder=self.data.n - i,
                    )  # dropna added
        else:
            ...
            if not self.rempli_polygone:
                for i, c in zip(self.data.ns, safe_colors):
                    if (
                        f"{i}-gon X" in self.data.df_2
                        and f"{i}-gon Y" in self.data.df_2
                    ):
                        self.ax.plot(
                            self.data.df_2[f"{i}-gon X"].dropna(),
                            self.data.df_2[f"{i}-gon Y"].dropna(),
    ...
    def plot_circle_inscribed(self, colors):
        if (
            not (self.show_ce_in and not self.show_ce_circon)
            or not hasattr(self.data, "centres")
            or not hasattr(self.data, "in_radii")
        ):
            return
        ...
        min_len = min(
            len(self.data.centres),
            len(self.data.in_radii),
            len(colors),
            len(contour_colors),
        )

        for pt, in_r, col, edge_col, i in zip(
            self.data.centres[:min_len],
            self.data.in_radii[:min_len],
            colors[:min_len],
            contour_colors[:min_len],
            range(min_len),
        ):
            if (
                isinstance(pt, (list, tuple))
                and len(pt) == 2
                and isinstance(in_r, (int, float))
                and in_r >= 0
            ):
                c = plt.Circle(
                    (pt[0] * -1, pt[1] * -1),
                    in_r,
                    fill=self.rempli_ce_in,
                    lw=self.line_thickness,
                    edgecolor=edge_col,
                    facecolor=col,
                    alpha=transp,
    ...
    def plot_circle_circumscribed(self, colors):
        if (
            not (self.show_ce_circon and not self.show_ce_in)
            or not hasattr(self.data, "centres")
            or not hasattr(self.data, "out_radii")
        ):
            return
        transp = 0.6 if self.rempli_polygone and self.show_polygone else 1
        contour_colors = [self.couleur] * len(colors) if self.contour else colors
        min_len = min(
            len(self.data.centres),
            len(self.data.out_radii),
            len(colors),
            len(contour_colors),
        )

        for pt, out_r, col, edge_col, i in zip(
            self.data.centres[:min_len],
            self.data.out_radii[:min_len],
            colors[:min_len],
            contour_colors[:min_len],
            range(min_len),
        ):
            if (
                isinstance(pt, (list, tuple))
                and len(pt) == 2
                and isinstance(out_r, (int, float))
                and out_r >= 0
            ):
                c = plt.Circle(
                    (pt[0] * -1, pt[1] * -1),
                    out_r,
                    fill=self.rempli_ce_circon,
                    lw=self.line_thickness,
                    edgecolor=edge_col,
                    facecolor=col,
                    alpha=transp,
    ...
    def plot_circle_inscribed_circumscribed(self, colors):
        if (
            not (self.show_ce_circon and self.show_ce_in)
            or not hasattr(self.data, "centres")
            or not hasattr(self.data, "in_radii")
            or not hasattr(self.data, "out_radii")
        ):
            return
        transp_inscrit, transp_circonscrit = (
            (1, 0.6) if self.rempli_ce_in and self.rempli_ce_circon else (1, 1)
        )
        ...
        # Draw circumscribed circles first (larger, behind)
        for pt, out_r, col, edge_col, i in zip(
            self.data.centres[:min_len],
            self.data.out_radii[:min_len],
            colors[:min_len],
            ...
            if (
                ...
                c_out = plt.Circle(
                    ...
                    edgecolor=edge_col,
                    facecolor=col,
                    alpha=transp_circonscrit,
                    zorder=self.data.n - i - 1,
                )
                self.ax.add_patch(c_out)

        # Draw inscribed circles second (smaller, in front)
        for pt, in_r, col, edge_col, i in zip(
            self.data.centres[:min_len],
            self.data.in_radii[:min_len],
            colors[:min_len],
    ...
    def plot_spiral(self):
        if not self.show_spirale:
            return
        if (
            not hasattr(self.data, "spiral_x")
            or not hasattr(self.data, "spiral_y")
            or not self.data.spiral_x
            or not self.data.spiral_y
        ):
            return
        to_check = [
            self.show_polygone,
        ...
        line_color = (
            self.couleur if self.contour and not any(to_check) else self.text_color
        )
        self.ax.plot(
            self.data.spiral_x,
            self.data.spiral_y,
    ...
    def plot_bezier(self, colors2):
        if (
            not self.show_curve
            or not hasattr(self.data, "bezier_pts")
            or not self.data.bezier_pts
        ):
            return
        if len(self.data.bezier_pts) < 2:
            return

        # Ensure colors2 has enough elements or handle mismatch
        num_segments = len(self.data.bezier_pts) - 1
        safe_colors2 = (
            colors2[:num_segments]
            if len(colors2) >= num_segments
            else ([colors2[0]] * num_segments if colors2 else ["blue"] * num_segments)
        )  # Fallback color
        ...
        if segments:  # Only add collection if valid segments exist
            lc = LineCollection(
                segments,
                colors=line_colors,
                linewidths=self.line_thickness * 1.5,
                zorder=self.data.n + 1,
            )
    ...
    def plot_circle(self, colors):
        if (
            not self.show_cercle
            or not hasattr(self.data, "ns")
            or not hasattr(self.data, "a")
            or not hasattr(self.data, "df_2")
        ):
            return
        patches, edge_colors = [], []
        ...
        for i, col in zip(self.data.ns[:min_len], base_colors[:min_len]):
            if f"{i}-gon X" in self.data.df_2 and f"{i}-gon Y" in self.data.df_2:
                ...
                for ptsx, ptsy in zip(xvals, yvals):
                    ...
                    if isinstance(ptsx, (int, float)) and isinstance(
                        ptsy, (int, float)
                    ):
    ...
    def plot_graph(self, colors):
        if not (self.show_graphe and hasattr(self.data, "ds") and self.data.ds != 0):
            return
        if (
            not hasattr(self.data, "ns")
            or not hasattr(self.data, "df_2")
            or not hasattr(self.data, "spiral_x")
            or not self.data.spiral_x
        ):
            return
        ...
        for i, col, ind in zip(
            self.data.ns[:min_len], line_colors[:min_len], range(min_len)
        ):
            if f"{i}-gon X" in self.data.df_2 and f"{i}-gon Y" in self.data.df_2:
                xvals = self.data.df_2[f"{i}-gon X"].dropna()
                yvals = self.data.df_2[f"{i}-gon Y"].dropna()
                for ptsx, ptsy in zip(xvals, yvals):
                    ...
                    if (
                        isinstance(ptsx, (int, float))
                        and isinstance(ptsy, (int, float))
                        and spiral_index < len(self.data.spiral_x)
                    ):
                        ...
                        if isinstance(spiral_pt_x, (int, float)) and isinstance(
                            spiral_pt_y, (int, float)
                        ):
                            line_segments.append(
                                [(spiral_pt_x, spiral_pt_y), (ptsx, ptsy)]
                            )
                            seg_colors.append(col)
        if line_segments:
            lc = LineCollection(
    ...
    def plot_segment(self):
        if not self.show_segment:
            return
        if (
            not hasattr(self.data, "ds")
            or self.data.ds <= 0
            or not hasattr(self.data, "spiral_x")
            or len(self.data.spiral_x) < 2
        ):
            return
        ...
        while count + 1 < nb:  # Iterate up to the second to last point
            ...
            if (
                isinstance(p1_x, (int, float))
                and isinstance(p1_y, (int, float))
                and isinstance(p2_x, (int, float))
                and isinstance(p2_y, (int, float))
            ):
        ...
        if segments:
            lc = LineCollection(
                ...
                linewidth=self.line_thickness,
                linestyle="-",
                zorder=self.data.n + 2 if hasattr(self.data, "n") else 2,
            )
            self.ax.add_collection(lc)
            self.ax.autoscale_view()
    ...
    def plot_point(self):
        ...
        if not valid_x:
            return  # No valid points to plot
...
def main():
    ...
    with st.sidebar:
        ...
        n_sides = st.slider(
            "Number of sides (n)", min_value=3, max_value=100, value=50, step=1
        )
        ds_offset = st.slider(
            "Spiral offset (ds)", min_value=0, max_value=3, value=1, step=1
        )
        ...
        display_final_choices = st.multiselect(
            "Elements to Display", display_options, default=["Graph", "Vertex"]
        )

        fillable_options = [
            opt
            for opt in ["Polygon", "Inscribed Circle", "Circumscribed Circle"]
            if opt in display_final_choices
        ]
        remplissage_final_choices = []
        if fillable_options:
            remplissage_final_choices = st.multiselect(
                "Elements to Fill", fillable_options, default=fillable_options
            )
    ...
    with tabs[0]:  # Generator
        if generate_button:
            ...
            with st.spinner("Generating your masterpiece..."):
                try:
                    ...
                    fig, data = (
                        plotter_instance.plot_fig()
                    )  # plot_fig no longer calculates
                    ...
                    if fig is not None:
                        ...
                        try:
                            st.session_state.figure_history.appendleft(
                                fig.copy()
                            )  # Add newest to the left
        ...
        if "fig" in st.session_state and st.session_state.fig is not None:
            ...
            st.info(
                "Adjust the settings in the sidebar and click 'Generate' to create your image."
            )
                    ...
                    if fig_index < num_figures:
                        with cols[j]:
                            try:
                                ...
                                st.error(
                                    f"Error displaying history figure {fig_index + 1}: {e}"
                                )
                    else:
    ...
    with tabs[2]:  # Data
        if "data" in st.session_state and st.session_state.data is not None:
            ...
            if hasattr(st.session_state.data, "df_1") and isinstance(
                st.session_state.data.df_1, pd.DataFrame
            ):
                df1_to_display = st.session_state.data.df_1.copy()  # Work on a copy
                numeric_cols_df1 = df1_to_display.select_dtypes(
                    include=np.number
                ).columns
                st.dataframe(df1_to_display)
            else:
                st.warning(
                    "General polygon information data (df_1) is not available or invalid."
                )

            st.markdown("### Polygon Vertex Coordinates")
            # Check if df_2 exists and is a DataFrame
            if hasattr(st.session_state.data, "df_2") and isinstance(
                st.session_state.data.df_2, pd.DataFrame
            ):
                df2_to_display = st.session_state.data.df_2.copy()  # Work on a copy
                numeric_cols_df2 = df2_to_display.select_dtypes(
                    include=np.number
                ).columns
                st.dataframe(df2_to_display)
            else:
                st.warning(
                    "Polygon vertex coordinate data (df_2) is not available or invalid."
                )
        else:
    ...
    with tabs[4]:
        ...
        iter_bezier = st.slider(
            "Bezier Subdivisions",
            0,
            8,
            4,
            key="iter_bezier",
            help="Number of recursive subdivisions (0 = control points only).",
        )
        show_controls = st.checkbox("Show Control Points", value=True)

        try:
            data_bezier = get_calculation_data(
                n=n_poly, ds=ds_bezier, reverse=False, bezier_iterations=iter_bezier
            )
            ...
            if (
                hasattr(data_bezier, "ns")
                and len(data_bezier.ns) > 0
                and hasattr(data_bezier, "df_2")
                and not data_bezier.df_2.empty
            ):
                all_x_list = [
                    data_bezier.df_2[f"{i}-gon X"].dropna()
                    for i in data_bezier.ns
                    if f"{i}-gon X" in data_bezier.df_2
                ]
                all_y_list = [
                    data_bezier.df_2[f"{i}-gon Y"].dropna()
                    for i in data_bezier.ns
                    if f"{i}-gon Y" in data_bezier.df_2
                ]
            ...
            if (
                hasattr(data_bezier, "spiral_x")
                and hasattr(data_bezier, "spiral_y")
                and data_bezier.spiral_x
                and data_bezier.spiral_y
            ):
                ax_bezier.plot(
                    data_bezier.spiral_x,
                    data_bezier.spiral_y,
                    color="#0077b6",
                    lw=1.5,
                    ls="--",
            ...
            if hasattr(data_bezier, "bezier_pts") and data_bezier.bezier_pts:
                ...
                valid_bezier_pts = [
                    p
                    for p in data_bezier.bezier_pts
                    if isinstance(p, (tuple, list)) and len(p) == 2
                ]
                if len(valid_bezier_pts) > 1:
                    bz_x, bz_y = zip(*valid_bezier_pts)
                    ax_bezier.plot(
                        bz_x, bz_y, color="#d00000", lw=2, label="Bezier Curve"
                    )
            ...
            if show_controls and hasattr(data_bezier, "pts") and data_bezier.pts:
                ...
                valid_pts = [
                    p
                    for p in data_bezier.pts
                    if isinstance(p, (tuple, list))
                    and len(p) == 3
                    and all(isinstance(pt, (tuple, list)) and len(pt) == 2 for pt in p)
                ]
                if valid_pts:
            ...
            if (
                plot_lims[0] is not None
                and plot_lims[1] is not None
                and y_center is not None
                and x_center is not None
            ):
                ax_bezier.plot(
                    plot_lims[1], y_center, ">", color="black", clip_on=False
                )
                ax_bezier.plot(
                    x_center, plot_lims[1], "^", color="black", clip_on=False
                )
            ax_bezier.legend()
    ...
    with tabs[6]:  # Help
        ...
        st.markdown(
            "This section displays the figures generated using the parameters set in the sidebar."
        )
        ...
        with col1:
            ...
            st.markdown(
                "Defines the maximum number of sides for the largest polygon. A higher `n` value creates a denser, more circular figure."
            )
        with col2:
            st.info("**Spiral offset (ds)**")
            st.markdown(
                "Controls how the vertices of adjacent polygons connect. `ds = 1` creates a smooth spiral, while `ds = 2` or `ds = 3` produces more complex patterns."
            )
        ...
        with col3:
            st.info("**Color Theme**")
            st.markdown(
                "`Random 1` selects a random predefined palette from the Matplotlib library. `Random 2` generates random colors to create color gradients."
            )
        with col4:
            st.info("**Elements to Display**")
            st.markdown(
                "Displays multiple figures based on the same principles—nested polygons with an offset between them. Multiple elements can be selected at once."
            )
        ...
        with col5:
            ...
            st.markdown(
                "When `Polygon`, `Inscribed Circle`, or `Circumscribed Circle` are selected under `Elements to Display`, you can choose whether to fill these shapes."
            )
        with col6:
            st.info("**Download Image**")
            st.markdown(
                "After generating a figure, the user can download the image directly through their browser."
            )

        st.markdown("---")
        st.markdown("### History")
        st.markdown(
            "This section displays the 21 most recent figures generated, with the latest shown first."
        )

        st.markdown("---")
        st.markdown("### Data")
        st.markdown(
            "For a detailed explanation of the parameters in the table, please refer to the accompanying paper."
        )
        st.markdown(
            "In the coordinate table, there is always one more line than the number of polygon sides because it also includes the order in which the polygon segments are drawn. Take the triangle (3-gon) for example, we start from the first coordinate point, draw to the second, then the third, and finally return to the first point. Therefore, the first and last coordinates are the same."
        )

        st.markdown("---")
        st.markdown("### Gallery")
        st.markdown(
            "The gallery includes images generated directly using this application, as well as some variations created by adjusting certain parameters to explore different visual results."
        )

        st.markdown("---")
        st.markdown("### Bézier Playground")
        st.markdown(
            "In this tab, users can experiment with the internal workings of the `Bézier curve` feature used in `Elements to Display`. While the main program uses a fixed number of iterations, this section allows you to explore how the Bézier algorithm behaves with different iteration counts."
        )

        st.markdown("---")
        st.markdown("### Paper")
        st.markdown(
            "This short paper explains how the coordinates of the polygons were calculated for different offsets, and draws an interesting resemblance with the nautilus shell. It explores various spiral types and compares them to those generated with BéZier curves — which, interestingly, are quite close to logarithmic spirals with a growth coefficient near √φ ≈ 1.272."
        )

        st.markdown("---")
        st.markdown("### Settings")
        st.markdown(
            "Users can adjust the settings from the three-dot menu at the top right corner. Options include changing the background theme and enabling wide mode, depending on the device being used."
        )
